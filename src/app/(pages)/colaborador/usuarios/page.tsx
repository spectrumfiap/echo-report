// src/app/(admin)/colaborador/gerenciar-usuarios/page.tsx
"use client";
...
export default function GerenciarUsuariosPage() {
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setNotification(null);
    const payloadForAdd = {
        nomeCompleto: userFormData.name, email: userFormData.email, password: userFormData.password,
        role: userFormData.role, status: userFormData.status,
        locationPreference: userFormData.locationPreference, subscribedAlerts: userFormData.subscribedAlerts,
    };
    const payloadForEdit = {
        nomeCompleto: userFormData.name, email: userFormData.email, role: userFormData.role, status: userFormData.status,
        locationPreference: userFormData.locationPreference, subscribedAlerts: userFormData.subscribedAlerts,
    ...
    try {
      if (modalAction === 'add') {
        if (!payloadForAdd.password) { throw new Error("Senha é obrigatória para adicionar novo usuário."); }
  ...
  if (!isAdmin && !isLoading) {
    router.push('/'); // Redireciona se não for admin após o carregamento
